...
Section Cut_Admissibility.
  ...
  Section Okada.
    ...
    Let mw_mono (X Y X' Y' : _ -> Prop) : X ⊆ X' -> Y ⊆ Y' -> X' ⊸ Y ⊆ X ⊸ Y'.
    Proof. apply magicwand_monotone; auto. Qed.

    Let inc1_prop (K : Type) (X Y : K -> Prop)  x : Y ⊆ X -> Y x -> X x.
    Proof. simpl; auto. Qed.

    Let cl_under_closed X Y : cl Y ⊆ Y -> X ⊆ Y -> cl X ⊆ Y.
    Proof. apply cl_closed; eauto. Qed.
 
    Lemma Okada_formula A : sg (A::nil) ⊆ ⟦A⟧ /\ ⟦A⟧ ⊆ ↓A.
    Proof.
  ...
  Theorem ill_cut_admissibility Γ A : Γ ⊢ A -> exists p : Γ ⊢ A, ill_cut_free p.
  Proof.
...
Check ill_cut_admissibility.
Print Assumptions ill_cut_admissibility.
