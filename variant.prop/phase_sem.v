...
(*  Section bang. *)
    ...
    Let J x := cl (sg e) x /\ sg x ∘ sg x ⊆ cl (sg x ∘ sg x ∘ sg x).

    Let In_J : forall x, cl (sg e) x -> sg x ∘ sg x ⊆ cl (sg x ∘ sg x ∘ sg x) -> J x.
    Proof. split; auto. Qed.

    Let J_inv x : J x -> unit x /\ sg x ∘ sg x ⊆ cl (sg x ∘ sg x ∘ sg x).
    Proof. auto. Qed.
