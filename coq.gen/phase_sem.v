...
Section Relational_phase_semantics.
  ...
  Proof.
    revert ll; apply Forall_type_rect; simpl; auto; red; auto.
  Qed.
  ...
  Proof. induction f as [ | [] | | [] ]; simpl; unfold bang; auto; red; auto. Qed.
  ...
  Section soundness.
    ...
    Notation "l '⊢' x" := (ill_proof s1 s2 l x) (at level 70, no associativity).
  ...
  Notation "l '⊢' x [ s1 , s2 ]" := (ill_proof s1 s2 l x) (at level 70, no associativity).
  ...
  Section ill_nc_soundness.
    ...
    Theorem ill_nc_soundness cut Γ a : Γ ⊢ a [false,cut] -> ⟬߭Γ⟭  ⊆ ⟦a⟧.
    Proof.
      intros H; apply ill_soundness_rec with (1 := H); trivial.
    Qed.
  ...
  Section ill_comm_soundness.
    ...
    Theorem ill_comm_soundness cut Γ a : Γ ⊢ a [true,cut] -> ⟬߭Γ⟭  ⊆ ⟦a⟧.
    Proof.
      intros H; apply ill_soundness_rec with (1 := H); trivial.
    Qed.

  End ill_comm_soundness.
