...
Require Import list_focus utils_tac list_perm.
...
Section Forall_type_rect.

  Variable (X : Type) (P : X -> Type) (Q : list X -> Type)
           (HQ0 : Q nil) (HQ1 : forall x l, P x -> (forall y, In_t y l -> P y) -> Q l -> Q (x::l)).
 
  Theorem Forall_type_rect l : (forall x, In_t x l -> P x) -> Q l.
  Proof.
    induction l as [ | x l IHl ]; intros Hl.
    + apply HQ0.
    + apply HQ1.
      1,2: intros; apply Hl; simpl; auto.
      apply IHl.
      intros; apply Hl; simpl; auto.
  Qed.

End Forall_type_rect.
...
Section Permutation_map_inv_t.

  Variable (X Y : Type) (eqY_dec : forall x y : Y, { x = y } + { x <> y }) (f : X -> Y).
  
  Let Q m1 m2 := forall l1, m1 = map f l1 -> { l2 | m2 = map f l2 /\ l1 ~p l2}.

  Let pmit : forall m1 m2, m1 ~p m2 -> Q m1 m2.
  Proof. 
    apply Permutation_rect with (1 := eqY_dec); unfold Q; clear Q.
    * intros [ | ]; exists nil; auto; discriminate.
  ...
  Fact Permutation_map_inv_t l m : map f l ~p m -> { l' | m = map f l' /\ l ~p l' }.
  Proof. intro; apply pmit with (2 := eq_refl); auto. Qed.

End Permutation_map_inv_t.
