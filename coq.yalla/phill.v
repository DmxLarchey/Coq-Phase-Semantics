...
Section Rules.
  ...
  Definition rule_right_implication_left :=
    (forall Γ Δ Σ C A B, def_rule_mul_right_imp_left Γ Δ Σ C A B (A ☮ B)).
  Definition rule_right_implication_right :=
    (forall Γ A B, def_rule_mul_right_imp_right Γ A B (A ☮ B)).
  Definition rule_left_implication_left :=
   (forall Γ Δ Σ C A B, def_rule_mul_left_imp_left Γ Δ Σ C A B (A ☮ B)).
  Definition rule_left_implication_right :=
   (forall Γ A B, def_rule_mul_left_imp_right Γ A B (A ☮ B)).
  Definition rule_right_negation_left :=
   (forall Γ Δ Σ C A, def_rule_mul_right_imp_left Γ Δ Σ C A Ret (☡ A)).
  Definition rule_right_negation_right :=
   (forall Γ A, def_rule_mul_right_imp_right Γ A Ret (☡ A)).
  Definition rule_left_negation_left :=
   (forall Γ Δ Σ C A, def_rule_mul_left_imp_left Γ Δ Σ C A Ret (☡ A)).
  Definition rule_left_negation_right :=
   (forall Γ A, def_rule_mul_left_imp_right Γ A Ret (☡ A)).
...
Section PhILL.
  ...
  Section PhaseSemantics.
    ...
    Fact pssem_tens_l : rule_tensor_left sem_jdgt (fun x y => x ∘ y).
    Proof. intros ? ? ? ? ? ?; apply sem_tens_l; assumption. Qed.

    Fact pssem_tens_r : rule_tensor_right sem_jdgt (fun x y => x ∘ y).
    Proof. intros ? ? ? ? ? ?; apply sem_tens_r; assumption. Qed.

    Fact pssem_one_l : rule_one_left sem_jdgt ε.
    Proof. intros ? ? ? ?; apply sem_one_l; assumption. Qed.

    Fact pssem_one_r : rule_one_right sem_jdgt ε.
    Proof. apply sem_one_r. Qed.

    Fact pssem_map_l : rule_right_implication_left sem_jdgt (fun x y => x ⇀ y).
    Proof. intros ? ? ? ? ? ? ? ?; apply sem_limp_l; assumption. Qed.

    Fact pssem_map_r : rule_right_implication_right sem_jdgt (fun x y => x ⇀ y).
    Proof. intros ? ? ? ?; apply sem_limp_r; assumption. Qed.

    Fact pssem_pam_l : rule_left_implication_left sem_jdgt (fun x y => y ↼ x).
    Proof. intros ? ? ? ? ? ? ? ?; apply sem_rimp_l; assumption. Qed.

    Fact pssem_pam_r : rule_left_implication_right sem_jdgt (fun x y => y ↼ x).
    Proof. intros ? ? ? ?; apply sem_rimp_r; assumption. Qed.

    Fact pssem_with_l1 : rule_with_left_1 sem_jdgt (fun x y => x ∩ y).
    Proof. intros ? ? ? ? ? ?; apply sem_with_l1; assumption. Qed.

    Fact pssem_with_l2 : rule_with_left_2 sem_jdgt (fun x y => x ∩ y).
    Proof. intros ? ? ? ? ? ?; apply sem_with_l2; assumption. Qed.

    Fact pssem_with_r : rule_with_right sem_jdgt (fun x y => x ∩ y).
    ...
    Fact pssem_oc_l : rule_dereliction_left sem_jdgt #.
    Proof. intros ? ? ? ? ?; apply sem_prebang_l; assumption. Qed.

    Fact pssem_oc_r : rule_promotion_right sem_jdgt #.
    Proof. intros ? ? ?; apply sem_prebang_r; assumption. Qed.

    Fact pssem_wk_l : rule_weakening_modal_monad sem_jdgt # □.
    Proof. intros ? ? ? ? ?; apply sem_prebang_weak; assumption. Qed.

    Fact pssem_co_l : rule_contraction_modal_monad sem_jdgt # □.
  ...
  Section SyntacticModel.
    ...
    Instance ctx_PS : MPhaseSpace _ := PS_ctx (cutrm_ipfrag P).
    Instance ctx_CL : ClosureOp _ := PSCL.
    ...
    change (B :: Δ) with ((B :: nil) ++ Δ); now apply H.
    Qed.
    ...
    change (N :: Δ) with ((N :: nil) ++ Δ); now apply H.
    Qed.
    ...
    Lemma sg_sem_l Σ Σ' :
     (forall Γ Δ C, Γ ++ Σ ++ Δ ⊢ C [cutrm_ipfrag P] -> Γ ++ Σ' ++ Δ ⊢ C [cutrm_ipfrag P]) ->
    ...
    intros [[Γ Δ] C] H; now apply Hrule, H.
    Qed.
    ...
    - now apply pssem_plus_l.
    Qed.

    Lemma sg_sem_l_nul Σ' : (forall Γ Δ C, Γ ++ Σ' ++ Δ ⊢ C [cutrm_ipfrag P]) ->
      forall Γ Δ C, Γ ++ ﹛Σ'﹜ :: Δ ⊧ □C.
    Proof.
    intros Hrule Γ Δ C.
    ...
    rewrite <- app_assoc; now apply sem_limp_l, sem_monad_l.
    Qed.
    ...
    now apply sem_rimp_l, sem_monad_l.
    Qed.
    ...
    Lemma ctx_sem_bin_r (f : forall {T}, list T -> list T -> list T)
                        (E : forall {T}, list T -> list T -> Type) c :
      (forall Γ Δ l, E Γ Δ -> list_compose ctx_PS (f Γ Δ) l ->
         {'(l1,l2):_ & (list_compose ctx_PS Γ l1 * list_compose ctx_PS Δ l2)%type
                     & ((l = f l1 l2) * E l1 l2)%type }) ->
      (forall Γ Δ A B, E Γ Δ -> Γ ⊢ A [cutrm_ipfrag P] -> Δ ⊢ B [cutrm_ipfrag P] ->
                                f Γ Δ ⊢ c A B [cutrm_ipfrag P]) ->
      forall Γ Δ A B, E Γ Δ -> Γ ⊧ ↓A -> Δ ⊧ ↓B -> f Γ Δ ⊧ ↓(c A B).
    Proof.
    intros Hf Hrule Γ Δ A B HE H1 H2 l H.
    apply Hf in H as [(l1,l2) [Hl1 Hl2] [Heq1 Heq2]]; subst; auto.
    Qed.

    Lemma ctx_sem_mul_bin_r c :
     (forall Γ Δ A B, def_rule_mul_bin_right (ill (cutrm_ipfrag P)) Γ Δ A B (c A B)) ->
      forall Γ Δ A B, def_rule_mul_bin_right (sem_jdgt) Γ Δ (↓A) (↓B) (↓(c A B)).
    Proof.
    intros Hrule Γ Δ A B H1 H2.
    apply ctx_sem_bin_r with (E := fun _ _ _ => True); auto.
    - clear; intros Γ Δ l _ Hl; apply list_compose_app in Hl; inversion Hl as [l1 l2 H1 H2 Heq].
      now exists (l1,l2).
    - clear - Hrule; intros; now apply Hrule.
    Qed.
    ...
    Proof.
    intros Hrule Γ A B H1 H2.
    apply ctx_sem_bin_r with (f := fun _ x y => x) (E := fun _ x y => x = y) (Δ := Γ); auto; intros; subst.
    - now exists (l,l).
    - now apply Hrule.
    Qed.
    ...
    Lemma atlist_from_gax : forall a, { l | fst (projT2 (ipgax (cutrm_ipfrag P)) a) = map ν l }.
    Proof.
    intros a.
    ...
    enough (forall l0 Γ a l1 l2, map ν l1 ++ map ν l0 = fst (projT2 (ipgax (cutrm_ipfrag P)) a) ->
              map snd Γ = map phvar l0 ->
              list_compose ctx_PS (map (fun x => left_sem (fst x) (snd x)) Γ) l2 ->
              { c | map ν l1 ++ l2 = fst (projT2 (ipgax (cutrm_ipfrag P)) c)
    ...
    - assert (at_ctx a a0) as [Ha | Ha]
      ...
      + destruct Ha as [c [Heq1' Heq2']]; subst.
        red in P_gax_cut.
        specialize P_gax_cut with c a' (map ν l1) (map ν l0).
    ...
    - apply (pssem_cut ctx_PS) with (A := left_sem false A).
      + change (left_sem false A) with (right_sem false A).
        now apply (IHpi1 (false,A) l2).
    ...
    - etransitivity; [ | apply closed_right_sem ].
      ...
      apply (@sem_prebang_swap _ ctx_PS), sem_monad_r.
    ...
    - rewrite <- Heq0; destruct (fst x); simpl.
      + etransitivity; [ | apply closed_right_sem ].
        apply sg_tens_l, sem_monad_r.
        specialize IHpi with A' (l0 ++ (true,A) :: (true,B) :: l2); list_simpl in IHpi.
        now apply IHpi.
      + apply (pssem_tens_l ctx_PS).
        specialize IHpi with A' (l0 ++ (false,A) :: (false,B) :: l2); list_simpl in IHpi.
        now apply IHpi.
    - destruct (fst A'); simpl.
      + apply ctx_tens_r.
        * now apply (IHpi1 (true,A) l0).
        * now apply (IHpi2 (true,B) l1).
      + apply (pssem_tens_r ctx_PS).
        * now apply (IHpi1 (false,A) l0).
        * now apply (IHpi2 (false,B) l1).
    - rewrite <- Heq0; destruct (fst x); simpl.
      + etransitivity; [ | apply closed_right_sem ].
        apply sg_one_l, sem_monad_r.
        specialize IHpi with A' (l0 ++ l2); list_simpl in IHpi.
        now apply IHpi.
      + apply (pssem_one_l ctx_PS).
        specialize IHpi with A' (l0 ++ l2); list_simpl in IHpi.
        now apply IHpi.
    ...
    - simpl in Heq3; rewrite <- Heq3; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      + apply sg_map_l.
        * now apply (IHpi1 (true,A) l2).
        * apply sem_monad_r.
          specialize IHpi2 with A' (l0 ++ (true,B) :: l4); list_simpl in IHpi2.
          now apply IHpi2.
      + apply (pssem_map_l ctx_PS).
        * now apply (IHpi1 (false,A) l2).
        * apply sem_monad_r.
          specialize IHpi2 with A' (l0 ++ (false,B) :: l4); list_simpl in IHpi2.
          now apply IHpi2.
    - destruct (fst A'); simpl.
      + now apply ctx_map_r, (IHpi (true,B) ((true,A) :: Γ')).
      + now apply (pssem_map_r ctx_PS), (IHpi (false,B) ((false,A) :: Γ')).
    - rewrite <- Heq0; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      + apply sg_pam_l.
        * now apply (IHpi1 (true,A) l3).
        * apply (@sem_monad_r _ ctx_PS).
          specialize IHpi2 with A' (l0 ++ (true,B) :: l4); list_simpl in IHpi2.
          now apply IHpi2.
      + apply (pssem_pam_l ctx_PS).
        * now apply (IHpi1 (false,A) l3).
        * apply sem_monad_r.
          specialize IHpi2 with A' (l0 ++ (false,B) :: l4); list_simpl in IHpi2.
          now apply IHpi2.
    - destruct (fst A'); simpl.
    ...
    - simpl in Heq3; rewrite <- Heq3; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      ...
      + apply (pssem_map_l ctx_PS).
        * now apply (IHpi1 (false,A) l2).
        * apply (@sem_monad_l _ ctx_PS), sem_monad_r.
          specialize IHpi2 with A' (l0 ++ (false,µ atN) :: l4); list_simpl in IHpi2.
          now apply IHpi2.
    - destruct (fst A'); simpl.
      + now apply ctx_neg_r, (IHpi (true,◻(µ atN)) ((true,A) :: Γ')).
      + now apply (pssem_map_r ctx_PS), (IHpi (false,◻(µ atN)) ((false,A) :: Γ')).
    - rewrite <- Heq0; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      + apply sg_gen_l.
        * now apply (IHpi1 (true,A) l3).
        * apply (@sem_monad_r _ ctx_PS).
   ...
   - etransitivity; [ | apply closed_right_sem ].
      ...
      + apply sg_with_l1, (@sem_monad_r _ ctx_PS).
        ...
        now apply IHpi.
      + apply (pssem_with_l1 ctx_PS), (@sem_monad_r _ ctx_PS).
    ...
    - etransitivity; [ | apply closed_right_sem ].
      ...
      + apply sg_with_l2, (@sem_monad_r _ ctx_PS).
        ...
        now apply IHpi.
      + apply (pssem_with_l2 ctx_PS),  (@sem_monad_r _ ctx_PS).
        specialize IHpi with A' (l0 ++ (false,B) :: l2); list_simpl in IHpi.
        now apply IHpi.
    - destruct (fst A'); simpl.
      + apply ctx_with_r.
        * now apply (IHpi1 (true,A) Γ').
        * now apply (IHpi2 (true,B) Γ').
      + apply (pssem_with_r ctx_PS).
        * now apply (IHpi1 (false,A) Γ').
        * now apply (IHpi2 (false,B) Γ').
    - destruct (fst A'); simpl.
    ...
    - rewrite <- Heq0; destruct (fst x); simpl.
      + etransitivity; [ | apply closed_right_sem ].
        ...
        * specialize IHpi2 with A' (l0 ++ (true,B) :: l2); list_simpl in IHpi2.
          now apply IHpi2.
      + apply (pssem_plus_l ctx_PS).
        * specialize IHpi1 with A' (l0 ++ (false,A) :: l2); list_simpl in IHpi1.
          now apply IHpi1.
        * specialize IHpi2 with A' (l0 ++ (false,B) :: l2); list_simpl in IHpi2.
          now apply IHpi2.
    - destruct (fst A'); simpl.
      + now apply ctx_plus_r1, (IHpi (true,A) Γ').
      + now apply (pssem_plus_r1 ctx_PS), (IHpi (false,A) Γ').
    - destruct (fst A'); simpl.
      + now apply ctx_plus_r2, (IHpi (true,B) Γ').
      + now apply (pssem_plus_r2 ctx_PS), (IHpi (false,B) Γ').
    - rewrite <- Heq0; destruct (fst x); simpl.
      + etransitivity; [ | apply closed_right_sem ].
        apply sg_zero_l.
      + apply (pssem_zero_l ctx_PS).
    - rewrite <- Heq0; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      + apply sg_oc_l, (@sem_monad_r _ ctx_PS).
        specialize IHpi with A' (l0 ++ (true,A) :: l2); list_simpl in IHpi.
        now apply IHpi.
      + apply (pssem_oc_l ctx_PS), sem_monad_r.
        specialize IHpi with A' (l0 ++ (false,A) :: l2); list_simpl in IHpi.
        now apply IHpi.
    - assert ({ Δ & list_compose ctx_PS (map (fun x => left_sem (fst x) (snd x)) Γ')
      ...
      + apply (@pssem_oc_r _ ctx_PS).
        etransitivity; [ apply Hoc | ].
        now apply (IHpi (false,A) Γ').
    - rewrite <- Heq0; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      + apply sg_wk_l, (@sem_monad_r _ ctx_PS).
        specialize IHpi with A' (l0 ++ l2); list_simpl in IHpi.
        now apply IHpi.
      + apply (pssem_wk_l ctx_PS), (@sem_monad_r _ ctx_PS).
        specialize IHpi with A' (l0 ++ l2); list_simpl in IHpi.
        now apply IHpi.
    - rewrite <- Heq0; destruct (fst x); simpl; (etransitivity; [ | apply closed_right_sem ]).
      + apply sg_co_l, (@sem_monad_r _ ctx_PS).
        ...
        now apply IHpi.
      + apply (pssem_co_l ctx_PS), (@sem_monad_r _ ctx_PS).
    ...
    - rewrite HeqA0; now apply ILLgax with a.
    Qed.

  End SyntacticModel.


  Notation "Γ ⊦ A [ P ]" := (PhILL P Γ A) (at level 70).
  Notation "l ⊧  x [ P ]" := (list_compose (ctx_PS P) l ⊆ x) (at level 70).
